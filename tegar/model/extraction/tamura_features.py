...
def get_tamura(image, lbp: bool=False) -> dict[str, np.float64]:
	if lbp != True:
	...
	return {
		"Coarseness": coarseness(img, 5),
		"Contrast": contrast(img),
		"Directionality": directionality(img),
		"Roughness": roughness(coarseness(img, 5), contrast(img))
	}
