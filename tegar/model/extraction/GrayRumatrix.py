...
from model.extraction.lbp_extraction import lbp_implementation
import cv2
...
class getGrayRumatrix:
    ...
    def set_img(self, image: cv2.Mat, lbp: bool = False):
        '''
        image: RGB image of cv2.MatLike
        lbp: boolean type, whether to use LBP to extract the image, default is False
        return: the numpy array of the image
        '''
        if lbp:
            self.data = lbp_implementation(image)
        else:
    ...
    def read_img(self, path: str="", lbp: bool=False):
        try:
            if lbp != True:
                img = Image.open(path)
