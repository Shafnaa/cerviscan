...
def lbp_implementation(image):
    img_bgr: cv2.Mat = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
...
def get_lbp_features(image: cv2.Mat) -> dict[str, np.float64]:
	lbp_image = lbp_implementation(image).flatten()
